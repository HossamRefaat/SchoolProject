...
public class AuthenticationController : AppControllerBase
{
    ...
    public async Task<IActionResult> Create([FromForm] RefreshTokenCommand command)
    {
        var res = await Mediator.Send(command);
        return NewResult(res);
    }
    ...
    public async Task<IActionResult> ValidateToken([FromQuery] AuthorizeUserQuerey command)
    {
        var res = await Mediator.Send(command);
        return NewResult(res);
    }
